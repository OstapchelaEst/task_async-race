...
export class RenderPage {
    async renderBaseBlock() {
      ...
      <main class="main">
         ...
         <div class="select-page">
            <button class="select-page__race _lock">GARAGE</button>
            <button class="select-page__winners">WINNERS</button>
         </div>
         <div class="race">
            <div class="cars-count"></div>
            <div class="garage-page">PAGE#</div>
            <div class="garage"></div>
            <div class="pagination">
               <button class="pagination__prev-garage _lock">Prev</button>   
               <button class="pagination__next-garage">Next</button>       
            </div>
            </div>
         </div>
      </main>
    ...
    renderInterface() {
        (<HTMLElement>document.querySelector('.race')).innerHTML =
            `
      <div class="race-interface">
               ...
               <div class="race-interface__buttons buttons-interface">
                  ...
                  <button class="buttons-interface__reset _race__lock">reset</button>
    ...
    async renderwinners() {
      ...
      <div class="winners hidden">
         ...
         <div class="winners__page">
            ...
            <div class="winners__table table-winners">
               <div class="table-winners__page"></div>
                  <div class="table-winners__inner">
                     <div class="table-winners__head">
                           <div class="table-winners__number">Number</div>
                           <div class="table-winners__car">Car</div>
                           <div class="table-winners__name">Name</div>
                           <div class="table-winners__wins">Wins</div>
                           <div class="table-winners__best-time">Best time (seconds)</div>
                     </div>    
                     <div class="table-winners__body"></div>
               </div>
            </div>
            <div class="pagination">
            <button class="pagination__prev-winners _lock">Prev</button>   
            <button class="pagination__next-winners">Next</button>       
         </div>
      </div>
      `;
        WINNERS.addListenersSortWinners();
    }
    renderWiner(id: number, color: string, name: string, wins: number, time: number) {
        (<HTMLElement>document.querySelector('.table-winners__body')).innerHTML += `
<div class="table-winners__winer winer-${id}">
<div class="table-winners__number">${id}</div>
<div class="table-winners__car"><svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="60.000000pt" height="25.000000pt"
...
<div class="table-winners__name">${name}</div>
<div class="table-winners__wins">${wins}</div>
<div class="table-winners__best-time">${time}s</div>
</div>
`;
    }
...
<div class="track-car" id="${id}">
   <p class="track-car__name">Car#${id}</p>
   <div class="track-car__head">
      ...
      <button class="track-car__remove-button" data-car="${id}">Remove</button>
   ...
   <div class="track-car__body">
      ...
      <label for="drive-car-${id}" class="track-car__label-drive _lock">B</label>
      
      <div class="track-car__body-pictyre">
      <div class="track-car__pictyre-${id}">
    ...
    async getTrack() {
        ...
        NEW_CARS.getCars()
            .then((data) => {
                NEW_PAGINATION.examinationNextBUtton(STORE_SAVE.page, 'garage', 8);
                data.car.forEach((e: { name: string; color: string; id: number }) =>
                    this.renderTrack(e.name, e.color, e.id)
                );
                RENDER_PAGE.renderCountCars();
            })
            .then(() => {
                NEW_CARS.addListenerDeleteCar();
                NEW_CARS.addListenerUppdateCarSelect();
                NEW_ENGINE.addEngineListener();
                NEW_ENGINE.addStopListener();
            });
    }
    async renderCountCars() {
        await NEW_CARS.getCars().then((data) => {
            (<HTMLElement>document.querySelector('.cars-count')).innerHTML = `Garage(${data.count})`;
        });
    }
    renderWinerLighBox(winer: string, data: IStart) {
        const LIGHT_BOX: HTMLElement = document.createElement('div');

        LIGHT_BOX.classList.add('light-box');
        LIGHT_BOX.innerHTML = `
      <p>Win ${winer}: ${(data.distance / data.velocity / 1000).toFixed(2)}s</p>
      `;
        document.body.append(LIGHT_BOX);
        (<HTMLElement>document.querySelector('.light-box')).addEventListener('click', () => {
            (<HTMLElement>document.querySelector('.light-box')).remove();
        });
    }
}
